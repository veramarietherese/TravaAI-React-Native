...
-- Compatibility-safe normalization for legacy enum/check-backed role/status columns.
-- We intentionally use shadow TEXT columns instead of ALTER TYPE so old defaults,
-- enum types, and checks cannot block the migration.
ALTER TABLE public.trip_members ADD COLUMN IF NOT EXISTS __trava_role_text text;
ALTER TABLE public.trip_members ADD COLUMN IF NOT EXISTS __trava_status_text text;

UPDATE public.trip_members
SET
  __trava_role_text = CASE
    WHEN lower(coalesce(role::text, '')) = 'owner' THEN 'owner'
    ELSE 'member'
  END,
  __trava_status_text = CASE
    WHEN lower(coalesce(status::text, '')) IN ('accepted', 'active', 'joined', 'owner') THEN 'accepted'
    WHEN lower(coalesce(status::text, '')) IN ('rejected', 'declined') THEN 'rejected'
    ELSE 'pending'
  END,
  created_at = coalesce(created_at, now()),
  updated_at = coalesce(updated_at, now());

ALTER TABLE public.trip_members DROP COLUMN role CASCADE;
ALTER TABLE public.trip_members DROP COLUMN status CASCADE;
ALTER TABLE public.trip_members RENAME COLUMN __trava_role_text TO role;
ALTER TABLE public.trip_members RENAME COLUMN __trava_status_text TO status;
...
UPDATE public.trip_members SET role = 'member' WHERE role IS NULL OR role NOT IN ('owner','member');
UPDATE public.trip_members SET status = 'pending' WHERE status IS NULL OR status NOT IN ('pending','accepted','rejected');
...
ALTER TABLE public.trip_members ADD CONSTRAINT trip_members_status_check CHECK (status IN ('pending','accepted','rejected'));
...
-- Compatibility-safe split method normalization.
ALTER TABLE public.expense_tracking ADD COLUMN IF NOT EXISTS __trava_split_method_text text;
UPDATE public.expense_tracking
SET __trava_split_method_text = CASE
  WHEN lower(coalesce(split_method::text, '')) IN ('equal','exact','payer_only') THEN lower(split_method::text)
  ELSE 'equal'
END;
ALTER TABLE public.expense_tracking DROP COLUMN split_method CASCADE;
ALTER TABLE public.expense_tracking RENAME COLUMN __trava_split_method_text TO split_method;
ALTER TABLE public.expense_tracking ALTER COLUMN split_method SET DEFAULT 'equal';
UPDATE public.expense_tracking SET split_method = 'equal' WHERE split_method IS NULL;
ALTER TABLE public.expense_tracking ALTER COLUMN split_method SET NOT NULL;

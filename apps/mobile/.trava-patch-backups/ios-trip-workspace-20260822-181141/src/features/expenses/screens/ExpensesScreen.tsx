...
export function ExpensesScreen() {
  ...
  const currencyCode = trip.currencyCode;

  async function shareInvoice(mode: "reminder" | "invoice" | "receipt") {
    const summary = [
      `TRAVA · ${tripName}`,
      `Shared trip expenses: ${money(totalShared, currencyCode)}`,
      `Average per traveler: ${money(avgPerPerson, currencyCode)}`,
      `Your current shared allocation: ${money(myShared, currencyCode)}`,
      "",
      ...latestShared.slice(0, 12).map((expense) => `${expense.expenseDate} · ${expense.title} · ${money(expense.amount, currencyCode)}`),
    ].join("\n");
    const prefix = mode === "reminder" ? "Payment reminder\n\n" : mode === "receipt" ? "TRAVA expense receipt\n\n" : "TRAVA trip invoice\n\n";
    await Share.share({ title: `${tripName} expenses`, message: prefix + summary });
  }

    ...
    <View style={styles.row}>
      {ITEMS.map((item) => {
        ...
        return <Pressable key={item.key} accessibilityRole="button" accessibilityLabel={item.label} accessibilityState={{selected:active}} onPress={()=>router.replace(item.href)} style={({pressed})=>[styles.item,pressed&&styles.pressed]}>
          <View style={item.key==="profile"?[styles.profileCircle,active&&styles.profileCircleActive]:undefined}><Ionicons name={active?item.activeIcon:item.icon} size={item.key==="profile"?22:25} color={active?(item.key==="discover"?"#3564F3":"#13203B"):"#70809A"}/></View>
...
const styles=StyleSheet.create({
  ...
  profileCircle:{width:36,height:36,borderRadius:18,alignItems:"center",justifyContent:"center",backgroundColor:"rgba(255,255,255,.70)",borderWidth:1,borderColor:"rgba(210,216,226,.88)"},profileCircleActive:{backgroundColor:"#FFFFFF",borderColor:"#C9CED7",boxShadow:"0 5px 12px rgba(43,49,61,.10)"},

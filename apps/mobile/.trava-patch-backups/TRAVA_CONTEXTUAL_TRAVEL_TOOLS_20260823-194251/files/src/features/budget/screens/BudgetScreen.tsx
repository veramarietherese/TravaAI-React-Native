  ...
  return <SafeAreaView style={s.safe} edges={["top"]}><StatusBar style="dark"/><ScreenShell tripId={tripId} title={trip.name || "Japan"}>
    <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}><View style={s.max}>
      ...
      <View style={s.metrics}>
        <Glass style={s.metric}><PremiumActionGlyph glyph="💰" size={48} colors={["#FFF8E9", "#FFDDB2", "#FFB781"]}/><View style={s.metricCopy}><Text style={s.metricLabel}>Remaining Budget</Text><Text style={s.metricValue}>₱{formatMoney(remaining)}</Text><Text style={s.metricSub}>of ₱{formatMoney(state.totalBudget)}</Text></View></Glass>
        <Glass style={s.metric}><PremiumActionGlyph glyph="↗️" size={48} colors={["#FFF0F6", "#F2D9FF", "#B8C8FF"]}/><View style={s.metricCopy}><Text style={s.metricLabel}>Total Spent</Text><Text style={s.metricValue}>₱{formatMoney(spent)}</Text><Text style={s.metricSub}>{state.expenses.length} saved expenses</Text></View></Glass>
      </View>

      <View style={s.actions}>
        <Action glyph="➕" label="Add expense" colors={["#FFF5E8", "#F9D7FF", "#BFD0FF"]} onPress={() => setEditor("new")}/>
        <Action glyph="↔️" label="Move" colors={["#EFFFF5", "#CFF4E4", "#9BE5C6"]} onPress={() => setMoveOpen(true)}/>
        <Action glyph="📤" label="Send" colors={["#EEF2FF", "#D7D9FF", "#AEA8F5"]} onPress={() => void sendBudget()}/>
        <Action glyph="💳" label="Top up" colors={["#FFF0F6", "#FFD3E6", "#FFA3C8"]} onPress={() => setAmountMode("topup")}/>
      </View>
...
function Action({ glyph, label, colors, onPress }: { glyph: string; label: string; colors: readonly [string, string, string]; onPress(): void }) {
  return <Pressable accessibilityRole="button" onPress={onPress} style={({ pressed }) => [s.action, pressed && s.pressed]}><PremiumActionGlyph glyph={glyph} size={54} colors={colors}/><Text style={s.actionLabel}>{label}</Text></Pressable>;
}

...
export function DiscoverMap({ places, selectedId, center, onSelect }: DiscoverMapProps) {
...
*{box-sizing:border-box}html,body,#map{width:100%;height:100%;margin:0}body{overflow:hidden;background:#eaf5ff;font-family:-apple-system,BlinkMacSystemFont,'SF Pro Display','Segoe UI',sans-serif}.leaflet-container{background:#eaf5ff}.leaflet-tile{filter:saturate(.72) brightness(1.08) contrast(.87) hue-rotate(1deg);opacity:.96}.leaflet-control-zoom,.leaflet-control-attribution{display:none}
.pin{position:relative;width:62px;height:74px;display:flex;justify-content:center;filter:drop-shadow(0 10px 13px rgba(36,63,110,.20));cursor:pointer}.tail{position:absolute;top:48px;width:18px;height:18px;background:#fff;transform:rotate(45deg);border-radius:2px}.bubble{position:relative;z-index:2;width:58px;height:58px;border-radius:29px;padding:4px;background:#fff;box-shadow:0 0 0 1px rgba(214,224,237,.95)}.bubble img{width:100%;height:100%;object-fit:cover;border-radius:25px}.pin.sel .bubble{box-shadow:0 0 0 7px rgba(81,132,255,.18);transform:scale(1.06)}
...
.locate{position:absolute;right:18px;bottom:18px;z-index:800;width:58px;height:58px;border:0;border-radius:29px;background:rgba(255,255,255,.97);box-shadow:0 9px 22px rgba(37,59,92,.15);color:#3b6cff;font-size:26px;cursor:pointer}.hint{position:absolute;left:18px;bottom:18px;z-index:700;max-width:250px;padding:10px 13px;border-radius:16px;background:rgba(255,255,255,.94);box-shadow:0 8px 18px rgba(37,59,92,.10);color:#40516f;font-size:12px;font-weight:700}.hint.hidden{display:none}
</style></head><body><div id="map"></div><div id="hint" class="hint">Search anywhere, then tap a result to move the map to the exact location.</div><button class="locate" id="locate">➤</button><script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script><script>
const pts=${data};const requestedCenter=${centerData};const fallback=requestedCenter?[requestedCenter.latitude,requestedCenter.longitude]:[10.3157,123.8854];const selected=pts.find(p=>p.selected);const map=L.map('map',{zoomControl:false,attributionControl:false,scrollWheelZoom:true,doubleClickZoom:true,dragging:true,touchZoom:true,minZoom:2});L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19}).addTo(map);const ll=[];pts.forEach(p=>{ll.push([p.latitude,p.longitude]);const html='<div class="pin '+(p.selected?'sel':'')+'"><div class="tail"></div><div class="bubble"><img src="'+esc(p.imageUrl)+'"></div></div>';const icon=L.divIcon({className:'',html,iconSize:[62,74],iconAnchor:[31,67]});const marker=L.marker([p.latitude,p.longitude],{icon,riseOnHover:true}).addTo(map);marker.bindTooltip(esc(p.name),{direction:'top',offset:[0,-60],opacity:.92});marker.on('click',()=>parent.postMessage({type:'trava-discover-select',id:p.id},'*'));});let current=fallback;const userIcon=L.divIcon({className:'',html:'<div class="user"></div>',iconSize:[50,50],iconAnchor:[25,25]});const userMarker=L.marker(current,{icon:userIcon,zIndexOffset:900}).addTo(map);

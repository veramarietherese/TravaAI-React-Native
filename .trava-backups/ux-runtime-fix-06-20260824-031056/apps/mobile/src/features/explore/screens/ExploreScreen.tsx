...
// eslint-disable-next-line react-hooks/set-state-in-effect -- Discover synchronizes its fallback map state with the persisted location preference
  ...
  useEffect(() => {
    ...
    if (text.length < 2) {
// eslint-disable-next-line react-hooks/set-state-in-effect -- clearing a too-short search query intentionally resets remote suggestion state
      setSuggestions([]);
      setLoading(false);
      return;
    }
  ...
  }, [preferences.placePhotos, query, userCoords]);
  ...
  async function chooseSearchResult(place: DiscoverPlace) {
    ...
    if (preferences.placePhotos) void hydratePlacePhotos([place], (resolved: DiscoverPlace[]) => {

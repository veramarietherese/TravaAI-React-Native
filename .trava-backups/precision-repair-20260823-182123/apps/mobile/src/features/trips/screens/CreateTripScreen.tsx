...
type PendingCollaborator = { email: string; role: "Editor" | "Viewer" };
...
export function CreateTripScreen() {
  ...
  function addCollaborator() {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      Alert.alert("Invite collaborator", "Enter the complete email address first.");
      return;
    }
    ...
    setCollaborators((current) => [...current, { email, role: "Editor" }]);
    setInviteEmail("");
  }
      ...
      <ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        <View style={styles.max}>
          ...
          <View style={styles.dateRow}>
            <DateField label="Start" date={startDate} active={calendarOpen && calendarTarget === "start"} onPress={() => { setCalendarTarget("start"); setCalendarOpen(true); setMonthCursor(startOfMonth(startDate)); }} />
            <DateField label="End" date={endDate} active={calendarOpen && calendarTarget === "end"} onPress={() => { setCalendarTarget("end"); setCalendarOpen(true); setMonthCursor(startOfMonth(endDate)); }} clearable onClear={() => setEndDate(addDays(startDate, 1))} />
          </View>
          ...
          <View style={styles.collabCard}>
            ...
            <View style={styles.inviteRow}>
              <View style={styles.inviteInputWrap}><Ionicons name="mail-outline" size={22} color="#64748D" /><TextInput value={inviteEmail} onChangeText={setInviteEmail} onSubmitEditing={addCollaborator} keyboardType="email-address" autoCapitalize="none" autoCorrect={false} placeholder="Invite by email…" placeholderTextColor="#A0A8B7" style={styles.inviteInput} /></View>
              <Pressable onPress={addCollaborator} style={styles.invitePress}><LinearGradient colors={["#87B9FF", "#A8A6F7", "#EE8DBB"]} start={{x:0,y:0}} end={{x:1,y:1}} style={styles.inviteButton}><Text style={styles.inviteButtonText}>Invite</Text></LinearGradient></Pressable>
            </View>
            ...
            {collaborators.map((item) => <MemberRow key={item.email} name={item.email.split("@")[0]} secondary={item.email} role={item.role} roleTone={item.role === "Editor" ? "editor" : "viewer"} editable={permissionMode} onRolePress={() => cycleRole(item.email)} onRemove={() => setCollaborators((current) => current.filter((entry) => entry.email !== item.email))} />)}

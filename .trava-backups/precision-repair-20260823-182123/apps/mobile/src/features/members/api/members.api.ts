...
export type ResolvedTraveler = { fullName: string; avatarUrl: string | null };
...
export async function resolveTripMemberEmail(tripId: string, email: string): Promise<ResolvedTraveler | null> {
  const result = await apiRequest<{ data: ResolvedTraveler | null }>(`/api/trips/${tripId}/members/resolve?email=${encodeURIComponent(email)}`);
  return result.data;
}

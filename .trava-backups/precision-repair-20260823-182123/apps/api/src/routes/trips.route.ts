...
tripsRouter.get("/:tripId/members/resolve", async (request: Request, response: Response, next: NextFunction) => {
  try {
    ...
    const { email } = parse(inviteSchema, { email: request.query.email });
    const { data: person, error } = await getSupabaseAdmin()
      .from("profiles")
    ...
    response.json({
      data: person
        ? { fullName: text(person.full_name) ?? "TRAVA traveler", avatarUrl: text(person.avatar_url) }
        : null,
    });
  } catch (error) {
    next(error);

...
export function ProfileScreen() {
  ...
  function handleMenuPress(key: (typeof MENU)[number]["key"]) {
    ...
    Alert.alert("TRAVA settings", "Your account is active. Additional profile and notification preferences are being prepared for this build.");
  }
        ...
        <View style={[styles.maxWidth, { width: contentWidth }]}>
          <LinearGradient colors={["#ECF3FF", "#FFF0F8"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.headerGlow}>
            <View style={styles.headerRow}>
              ...
              <View style={styles.headerActions}><CircleIconButton icon="sparkles-outline" label="Open TRAVA AI" onPress={() => router.navigate("/(traveler)/(tabs)/ai" as Href)} /><CircleIconButton icon="settings-outline" label="Settings" onPress={() => handleMenuPress("settings")} /></View>
            </View>
...
const styles = StyleSheet.create({
  ...
  miniMascot: { position: "absolute", right: 58, bottom: 25, width: 108, height: 112, alignItems: "center", justifyContent: "center", transform: [{ rotate: "3deg" }] },
  streakMascotImage: { width: 108, height: 112 },

...
import { HomeDashboardSkeleton } from "../components/HomeDashboardSkeleton";
...
import { useRealtimeHomeNotifications } from "../hooks/useRealtimeHomeNotifications";
...
export function HomeScreen() {
  ...
  const { preferences } = useTravaPreferences();
  useRealtimeHomeNotifications(user?.id, dashboard.refresh, preferences.realtimeNotifications);
  ...
  const cardWidth = Math.min(344, Math.max(306, width - contentPadding * 2 - 28));
  const agencyWidth = Math.min(344, Math.max(306, width - contentPadding * 2 - 28));
  ...
  function handleQuickAction(action: HomeQuickActionKey) {
    ...
    const tripBase = `/trip/${encodeURIComponent(String(trip.id))}`;
    const routeByAction: Record<HomeQuickActionKey, string> = {
      itinerary: `${tripBase}/itinerary`,
      budget: `${tripBase}/budget`,
      expenses: `${tripBase}/expenses`,
      checklist: `${tripBase}/checklist`,
      documents: `${tripBase}/documents`,
    };
    router.push(typedHref(routeByAction[action]));
  }
    ...
    return (
      <SafeAreaView style={styles.safe} edges={["top"]}>
        ...
        <ScrollView
          ...
          refreshControl={<RefreshControl refreshing={dashboard.isRefreshing} onRefresh={() => void dashboard.refresh()} tintColor="#7695E8" />}
    ...
    <SafeAreaView style={styles.safe} edges={["top"]}>
      ...
      >
        <View style={styles.contentWidth}>
          ...
          <SectionHeader actionLabel="View All" onActionPress={() => openDirectory("tours")} title="Tour Packages for You" />
          {data.tours.length ? (
          ...
          <TravelAgencyPromoBanner agency={data.agencies[0] ?? null} onPress={() => openDirectory("agencies")} />
...
const styles = StyleSheet.create({
  ...
  loadingStar: { width: 74, height: 74, alignItems: "center", justifyContent: "center", transform: [{ rotate: "45deg" }], borderRadius: 22, },
  loadingStarCutout: { width: 18, height: 18, backgroundColor: "rgba(255,255,255,0.88)", borderRadius: 5 },
  loadingIconBubble: { position: "absolute", alignItems: "center", justifyContent: "center", borderRadius: 26, borderWidth: 1, borderColor: "rgba(255,255,255,0.9)", backgroundColor: "rgba(255,255,255,0.74)", },
  ...
  loadingAvatarBubble: { position: "absolute", width: 66, height: 66, alignItems: "center", justifyContent: "center", borderRadius: 33, borderWidth: 2, borderColor: "rgba(255,255,255,0.92)", backgroundColor: "rgba(255,255,255,0.82)", },
  avatarLeft: { left: 8, top: 192 },
  avatarRight: { right: 6, top: 198 },
  avatarBottom: { bottom: 2, left: 138 },
  loadingAvatarEmoji: { fontSize: 29 },
  loadingSparkle: { position: "absolute", width: 8, height: 8, borderRadius: 999, backgroundColor: "rgba(255,255,255,0.96)", },
  ...
  emptyResultsIcon: { color: "#7695E8", fontSize: 31, fontWeight: "900" },

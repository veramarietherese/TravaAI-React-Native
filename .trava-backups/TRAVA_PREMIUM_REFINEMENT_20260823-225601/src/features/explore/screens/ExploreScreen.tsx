...
const CATEGORIES: Array<{ name: Category; emoji: string; api: string }> = [
  { name: "Cafes", emoji: "☕", api: "cafes" },
  { name: "Food", emoji: "🍴", api: "food" },
  { name: "Shopping", emoji: "🛍️", api: "shopping" },
  { name: "Hiking", emoji: "🥾", api: "hiking" },
  { name: "Work", emoji: "💼", api: "work" },
  { name: "Parks", emoji: "🌲", api: "parks" },
];
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (permission.status !== "granted") { if (mounted) setUserCoords({ latitude: 10.3157, longitude: 123.8854 }); return; }
        const position = await Location.getCurrentPositionAsync({ accuracy: Location.Accuracy.Balanced });
        if (mounted) setUserCoords({ latitude: position.coords.latitude, longitude: position.coords.longitude });
      } catch { if (mounted) setUserCoords({ latitude: 10.3157, longitude: 123.8854 }); }
    })();
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        setSuggestions(result.map((item, index) => mapWorldPlace(item, inferCategory(text), index, userCoords)));
      } finally {
    ...
    <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={styles.scroll}>
      <View style={styles.max}>
        ...
        <View style={styles.searchArea}>
          <View style={[styles.search, searchOpen && query.trim().length >= 2 && styles.searchFocused]}>
            ...
            <TextInput
              ...
              placeholder="Search Cebu, Manila, cafes, hotels, landmarks…"
        ...
        <View style={styles.mapSection}>
          ...
          {categoryLoading ? <View style={styles.mapLoading}><ActivityIndicator color="#4F73FF" /><Text style={styles.mapLoadingText}>Finding real places nearby…</Text></View> : null}
        </View>

        {selected ? <View style={styles.selectedBar}>
          <View style={styles.selectedCopy}><Text numberOfLines={1} style={styles.selectedName}>{selected.name}</Text><Text numberOfLines={1} style={styles.selectedAddress}>{selected.subtitle}</Text></View>
          <Pressable onPress={() => void toggleSaved(selected)} style={[styles.pinButton, saved.includes(selected.id) && styles.pinButtonOn]}><Ionicons name={saved.includes(selected.id) ? "bookmark" : "bookmark-outline"} size={18} color={saved.includes(selected.id) ? "#2F63E9" : "#51617A"} /><Text style={[styles.pinButtonText, saved.includes(selected.id) && styles.pinButtonTextOn]}>{saved.includes(selected.id) ? "Pinned" : "Pin"}</Text></Pressable>
          <Pressable onPress={() => void openAddToItinerary()} style={styles.addButton}><Ionicons name="calendar-outline" size={18} color="#FFF" /><Text style={styles.addButtonText}>Add to itinerary</Text></Pressable>
        </View> : <View style={styles.emptyPrompt}><Ionicons name="globe-outline" size={20} color="#71819A" /><Text style={styles.emptyPromptText}>Search worldwide or tap directly on the map to drop a pin. You can save it or add it to a trip itinerary.</Text></View>}
        ...
        {selected ? <View style={styles.featured}>
          ...
          <View style={styles.featuredCopy}>
            <Text style={styles.featuredTag}>✦ Featured nearby</Text>
            ...
            <View style={styles.featuredActions}><Pressable onPress={() => void toggleSaved(selected)} style={styles.featuredSecondary}><Ionicons name="bookmark-outline" size={17} color="#3658C8" /><Text style={styles.featuredSecondaryText}>Pin</Text></Pressable><Pressable onPress={() => void openAddToItinerary()} style={styles.detailsPress}><LinearGradient colors={["#3D6FFF", "#B073FA", "#FF7BB1"]} start={{ x: 0, y: .5 }} end={{ x: 1, y: .5 }} style={styles.detailsButton}><Text style={styles.detailsText}>Add to itinerary</Text><Ionicons name="chevron-forward" size={18} color="#FFF" /></LinearGradient></Pressable></View>
          </View>
        </View> : null}
      </View>
    </ScrollView>
...
function mapWorldPlace(item: WorldPlaceResult, category: Category, index: number, origin?: Coordinates | null): DiscoverPlace {
  return {
    id: item.id,
    name: item.name,

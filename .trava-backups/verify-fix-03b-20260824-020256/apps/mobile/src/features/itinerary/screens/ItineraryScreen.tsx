  ...
  return <SafeAreaView style={s.safe} edges={["top"]}><StatusBar style="dark"/><ScreenShell tripId={tripId} title={trip.name || "Japan"}>
    <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}><View style={s.max}>
      ...
      <View style={s.timelineCard}>
        <View style={s.timelineHead}><View><Text style={s.dayTitle}>Day {day} · {dayLabel(trip.startDate, day)}</Text><Text style={s.daySubtitle}>{dayItems.length ? `${dayItems.length} scheduled ${dayItems.length === 1 ? "activity" : "activities"}` : "Nothing scheduled yet"}</Text></View><View style={s.weather}><Ionicons name={weatherIcon(weather?.label)} size={17} color="#5E7392"/><Text style={s.weatherText}>{weather?.temperature != null ? `${Math.round(weather.temperature)}°C` : "Weather"}</Text></View></View>{weather ? <View style={s.weatherTip}><Ionicons name="bulb-outline" size={17} color="#6C7D99"/><View style={{flex:1}}><Text style={s.weatherTipTitle}>{weather.label}</Text><Text style={s.weatherTipText}>{weather.tip}</Text></View></View> : null}
...
function weatherIcon(label?: string): TravaIconName {
  ...
  if (value.includes("cloud")) return "cloud-outline";

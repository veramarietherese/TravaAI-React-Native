...
export function TravaGlassNav({ placement = "floating" }: { placement?: "floating" | "tabbar" }) {
          ...
          if (item.key === "ai") {
            return (
              ...
              >
                ...
                >
                  <View style={[styles.aiHighlight, { pointerEvents: "none" }]} />
  ...
  if (placement === "tabbar") {
    return <View style={[{ pointerEvents: "box-none" }, styles.tabbarHost, { paddingBottom: Math.max(8, insets.bottom) }]}>{bar}</View>;
  }
  return <View style={[{ pointerEvents: "box-none" }, styles.floatingHost, { bottom: Math.max(8, insets.bottom + 5) }]}>{bar}</View>;
}

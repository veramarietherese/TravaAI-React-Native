  ...
  return (
    <View style={styles.section}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.pulseRow}>
        <PulseCard icon="calendar-outline" colors={["#FFC4D2", "#F3B9D8", "#C7D4FF"]} value={countdown === null ? "Dates" : countdown <= 0 ? "Now" : `${countdown}d`} label="Trip timing" detail={trip.startDate ? formatTripDate(trip.startDate, trip.endDate) : "Set your dates"} onPress={onTrip} />
        <PulseCard icon="checkmark-done-outline" colors={["#D8C7FF", "#C8C5F8", "#B6D1FF"]} value={checklistTotal ? `${checklistPercent}%` : "Start"} label="Checklist" detail={checklistTotal ? `${checklistCompleted} of ${checklistTotal} completed` : "Build your travel checklist"} onPress={onChecklist} />
        <PulseCard icon="documents-outline" colors={["#B8D9FF", "#BFD0F8", "#D7C4F6"]} value={documentCount ? `${documentCount}` : "Add"} label="Documents" detail={documentCount ? `${documentCount} saved securely` : "Passport, tickets & more"} onPress={onDocuments} />
        <PulseCard icon="wallet-outline" colors={["#FFD6AE", "#F8BCC6", "#E0C4F7"]} value={trip.totalBudget > 0 ? `${budgetPercent}%` : "Set"} label="Budget left" detail={trip.totalBudget > 0 ? formatMoney(budgetRemaining, trip.currencyCode) : "Create a trip budget"} onPress={onBudget} />
        <PulseCard icon="people-outline" colors={["#C4DEFF", "#C9CEF8", "#E9C9F2"]} value={`${Math.max(1, trip.memberCount)}`} label="Travelers" detail="People on this trip" onPress={onTrip} />
      </ScrollView>
    </View>
  );
}
  ...
  return (
    <View style={styles.section}>
      ...
      <Pressable accessibilityRole="button" onPress={onOpen} style={({ pressed }) => [styles.budgetCard, pressed && styles.pressed]}>
        <SoftIcon icon="wallet-outline" colors={["#F8C5D1", "#D0C4F2", "#B9D7F8"]} size={58} />

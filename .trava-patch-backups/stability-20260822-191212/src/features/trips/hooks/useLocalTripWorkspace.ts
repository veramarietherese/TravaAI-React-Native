...
export function useLocalTripWorkspace(tripId: string) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (alive) setState(raw ? ({ ...cloneSeed(), ...(JSON.parse(raw) as Partial<WorkspaceState>) }) : cloneSeed());
      } catch {
  ...
  const api = useMemo(() => ({
    ...
    addExpense(expense: Omit<LocalExpense, "id">) { commit((s) => ({ ...s, expenses: [{ ...expense, id: `e-${Date.now()}` }, ...s.expenses] })); },
    ...
    addChecklist(title: string, category: LocalChecklistItem["category"]) { commit((s) => ({ ...s, checklist: [{ id: `c-${Date.now()}`, title, category, completed: false }, ...s.checklist] })); },
    ...
    addActivity(activity: Omit<LocalActivity, "id">) { commit((s) => ({ ...s, activities: [...s.activities, { ...activity, id: `a-${Date.now()}` }] })); },
    ...
    addDocument(title: string, type = "Document") { commit((s) => ({ ...s, documents: [{ id: `d-${Date.now()}`, title, type, size: "Local", updated: "Today" }, ...s.documents] })); },
    deleteDocument(id: string) { commit((s) => ({ ...s, documents: s.documents.filter((d) => d.id !== id) })); },
    reset() { const fresh = cloneSeed(); setState(fresh); void AsyncStorage.setItem(key(tripId), JSON.stringify(fresh)); },
  }), [commit, tripId]);

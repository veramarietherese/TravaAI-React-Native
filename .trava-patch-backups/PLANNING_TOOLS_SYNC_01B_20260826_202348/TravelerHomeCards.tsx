...
export function ActiveTripCard({
  ...
  return (
    <Pressable accessibilityRole="button" accessibilityLabel={`Open ${trip.name}`} onPress={onOpen} style={({ pressed }) => [styles.tripCard, pressed && styles.pressed]}>
      {trip.imageUrl ? (
        <Image source={{ uri: trip.imageUrl }} contentFit="cover" style={StyleSheet.absoluteFillObject} />
      ) : (
        <LinearGradient colors={["#E8EDFF", "#F9E6F4", "#E8F5FF"]} style={StyleSheet.absoluteFillObject} />
      )}
...
function PulseCard({
  ...
  return <Pressable accessibilityRole="button" onPress={onPress} style={({ pressed }) => pressed && styles.pressed}>{body}</Pressable>;
}
  ...
  return (
    <View style={styles.section}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.pulseRow}>
        <PulseCard icon="calendar-outline" colors={["#FFB272", "#EF75B6", "#9D79F1"]} value={countdown === null ? "Dates" : countdown <= 0 ? "Now" : `${countdown}d`} label="Trip timing" detail={trip.startDate ? formatTripDate(trip.startDate, trip.endDate) : "Set your dates"} onPress={onTrip} />
        <PulseCard icon="checkmark-done-outline" colors={["#FC8EAD", "#B576F2", "#718DFF"]} value={checklistTotal ? `${checklistPercent}%` : "Start"} label="Checklist" detail={checklistTotal ? `${checklistCompleted} of ${checklistTotal} completed` : "Build your travel checklist"} onPress={onChecklist} />
        <PulseCard icon="documents-outline" colors={["#7898FF", "#6974EF", "#A66CE9"]} value={documentCount ? `${documentCount}` : "Add"} label="Documents" detail={documentCount ? `${documentCount} saved securely` : "Passport, tickets & more"} onPress={onDocuments} />
        <PulseCard icon="wallet-outline" colors={["#FFAF63", "#F47D8C", "#B776F4"]} value={trip.totalBudget > 0 ? `${budgetPercent}%` : "Set"} label="Budget left" detail={trip.totalBudget > 0 ? formatMoney(budgetRemaining, trip.currencyCode) : "Create a trip budget"} onPress={onBudget} />
        <PulseCard icon="people-outline" colors={["#7CB5FF", "#7E83F7", "#D276E8"]} value={`${Math.max(1, trip.memberCount)}`} label="Travelers" detail="People on this trip" onPress={onTrip} />
      </ScrollView>
    </View>
  );
}
...
export function AiConciergeCard({
  ...
  return (
    <LinearGradient colors={["rgba(250,245,255,0.98)", "rgba(243,247,255,0.98)", "rgba(255,244,250,0.98)"]} style={styles.aiCard}>
      <LinearGradient colors={["#F0B0FF", "#9B86FF", "#6FA9FF"]} style={styles.aiOrb}><View style={styles.aiFace}><View style={styles.aiEyeRow}><View style={styles.aiEye} /><View style={styles.aiEye} /></View><View style={styles.aiSmile} /></View><Ionicons name="sparkles" size={13} color="#FFFFFF" style={styles.aiSparkle} /></LinearGradient>
...
const SHORTCUTS: Array<{ key: string; label: string; icon: keyof typeof Ionicons.glyphMap; colors: readonly [string, string, ...string[]] }> = [
  { key: "itinerary", label: "Itinerary", icon: "map-outline", colors: ["#FF8DB5", "#BC77EE", "#748FFF"] },
  { key: "budget", label: "Budget", icon: "wallet-outline", colors: ["#6E9FFF", "#6585F0", "#A16FE3"] },
  { key: "expenses", label: "Expenses", icon: "receipt-outline", colors: ["#B08AF7", "#7C8DF4", "#69A7FA"] },
  { key: "checklist", label: "Checklist", icon: "checkmark-circle-outline", colors: ["#FF98B1", "#BD79ED", "#788BF5"] },
  { key: "documents", label: "Documents", icon: "documents-outline", colors: ["#8188F5", "#666CE0", "#A46FE5"] },
];

export function PlanningShortcuts({ onOpen }: { onOpen(key: string): void }) {
  return (
    <View style={styles.section}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.shortcutRow}>
        {SHORTCUTS.map((item) => <Pressable key={item.key} accessibilityRole="button" accessibilityLabel={`Open ${item.label}`} onPress={() => onOpen(item.key)} style={({ pressed }) => [styles.shortcutCard, pressed && styles.pressed]}><SoftIcon icon={item.icon} colors={item.colors} size={54} /><Text style={styles.shortcutText}>{item.label}</Text></Pressable>)}
      </ScrollView>
    </View>
  );
}
  ...
  return (
    <View style={styles.section}>
      ...
      <Pressable accessibilityRole="button" onPress={onOpen} style={({ pressed }) => [styles.budgetCard, pressed && styles.pressed]}>
        <SoftIcon icon="wallet-outline" colors={["#FDA7B3", "#A07DF3", "#75A1FF"]} size={58} />
...
const styles = StyleSheet.create({
  ...
  softIcon: { overflow: "hidden", alignItems: "center", justifyContent: "center", borderWidth: 1, borderColor: "rgba(255,255,255,0.88)", boxShadow: "0 8px 18px rgba(91,78,171,0.18)" },
  softHighlight: { position: "absolute", width: "76%", height: "36%", borderRadius: 20, top: 3, left: 5, backgroundColor: "rgba(255,255,255,0.25)" },
  ...
  pulseRow: { gap: 10, paddingTop: 10, paddingRight: 20 },
  pulseCard: { width: 132, minHeight: 154, padding: 13, borderRadius: 24, borderWidth: 1, borderColor: "rgba(255,255,255,0.96)", backgroundColor: "rgba(255,255,255,0.72)", boxShadow: "0 10px 26px rgba(72,65,130,0.07)" },
  ...
  shortcutRow: { gap: 10, paddingTop: 10, paddingRight: 20 },
  shortcutCard: { width: 100, minHeight: 116, alignItems: "center", justifyContent: "center", gap: 10, borderRadius: 23, borderWidth: 1, borderColor: "rgba(255,255,255,0.96)", backgroundColor: "rgba(255,255,255,0.72)", boxShadow: "0 10px 25px rgba(70,63,126,0.06)" },

...
import { GradientPill, PX, ScreenShell, type TravaIconName } from "@/features/trips/components/TravaPixelUI";
...
export function ItineraryScreen() {
  ...
  const maxDay = useMemo(() => Math.max(3, day, ...state.activities.map((item) => item.dayNumber)), [day, state.activities]);
  ...
  return <SafeAreaView style={s.safe} edges={["top"]}><StatusBar style="dark"/><ScreenShell tripId={tripId} title={trip.name || "Japan"}>
    <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}><View style={s.max}>
      <View style={s.mapCard}>
        <View style={s.mapLabel}><Ionicons name="map-outline" size={15} color="#5579A9"/><View><Text style={s.mapLabelTitle}>Interactive itinerary map</Text><Text style={s.mapLabelSub}>Drag, zoom, and tap a numbered stop.</Text></View></View>
      ...
      <View><Text style={s.controlTitle}>Choose a travel day</Text><Text style={s.controlSub}>Each day shows its own activities and schedule.</Text></View>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.days}>
        {Array.from({ length: maxDay }, (_, index) => index + 1).map((d) => {
          const count = state.activities.filter((item) => item.dayNumber === d).length;
          const date = dayLabel(trip.startDate, d);
          return <Pressable key={d} accessibilityLabel={`Open day ${d}, ${date}`} onPress={() => setDay(d)} style={s.daySlot}>{day === d ? <GradientPill style={s.dayOn}><Text style={s.dayOnKicker}>DAY {d}</Text><Text style={s.dayOnText}>{date}</Text><Text style={s.dayOnCount}>{count} {count === 1 ? "activity" : "activities"}</Text></GradientPill> : <View style={s.dayOff}><Text style={s.dayOffKicker}>DAY {d}</Text><Text style={s.dayOffText}>{date}</Text><Text style={s.dayOffCount}>{count} {count === 1 ? "activity" : "activities"}</Text></View>}</Pressable>;
        })}
        <Pressable onPress={() => setDay(maxDay + 1)} style={s.addDay}><Ionicons name="add" size={20} color="#6D91CF"/><Text style={s.addDayText}>New day</Text></Pressable>
      </ScrollView>

      <View style={s.timelineCard}>
        <View style={s.timelineHead}><View><Text style={s.dayTitle}>Day {day} · {dayLabel(trip.startDate, day)}</Text><Text style={s.daySubtitle}>{dayItems.length ? `${dayItems.length} scheduled ${dayItems.length === 1 ? "activity" : "activities"}` : "Nothing scheduled yet"}</Text></View><View style={s.weather}><Ionicons name="partly-sunny-outline" size={16} color="#7B91B3"/><Text style={s.weatherText}>18°C</Text></View></View>
        ...
        {dayItems.map((a, i) => {
          ...
          return <View key={a.id} style={s.row}><View style={s.timeCol}><Text style={s.time}>{clock(a.startTime)}</Text></View><View style={s.rail}><View style={s.dot}><View style={s.dotInner}/></View>{i < dayItems.length - 1 ? <View style={s.line}/> : null}</View><View style={[s.activity, selected === a.id && s.activityOn]}><Pressable onPress={() => setSelected(a.id)} style={s.activityMain}><View style={[s.activityIcon, { backgroundColor: meta.bg }]}><Ionicons name={meta.icon} size={24} color={meta.fg}/></View><View style={s.activityCopy}><Text style={s.activityTitle}>{a.title}</Text><Text style={s.activityPlace}>{a.locationName}</Text>{a.detail ? <Text style={s.activityDetail}>{a.detail}</Text> : null}</View>{a.estimatedCost > 0 ? <Text style={s.cost}>${a.estimatedCost}</Text> : null}</Pressable><Pressable accessibilityLabel={`Edit ${a.title}`} onPress={() => setEditor(a)} style={s.edit}><Ionicons name="create-outline" size={19} color="#6D82A4"/></Pressable></View></View>;
        })}
    ...
    <ActivityModal key={`${editor === "new" ? "new" : editor?.id ?? "closed"}-${day}`} value={editor} day={day} onClose={() => setEditor(null)} onSave={(input) => {
...
const s = StyleSheet.create({
  ...
  controlTitle: { color: PX.ink, fontSize: 15, fontWeight: "900" }, controlSub: { marginTop: 3, color: PX.muted, fontSize: 10, fontWeight: "600" }, days: { gap: 10 }, daySlot: { width: 154, height: 78 }, dayOn: { flex: 1, alignItems: "flex-start", paddingHorizontal: 16, borderRadius: 22 }, dayOnKicker: { color: "rgba(255,255,255,.78)", fontSize: 8, fontWeight: "900", letterSpacing: .8 }, dayOnText: { marginTop: 4, color: "#FFF", fontSize: 13, fontWeight: "900" }, dayOnCount: { marginTop: 3, color: "rgba(255,255,255,.85)", fontSize: 9, fontWeight: "700" }, dayOff: { flex: 1, paddingHorizontal: 16, justifyContent: "center", borderRadius: 22, backgroundColor: "#FFF", borderWidth: 1, borderColor: "#E8ECF3" }, dayOffKicker: { color: "#94A0B3", fontSize: 8, fontWeight: "900", letterSpacing: .8 }, dayOffText: { marginTop: 4, color: "#43516E", fontSize: 13, fontWeight: "900" }, dayOffCount: { marginTop: 3, color: "#8B96A9", fontSize: 9, fontWeight: "700" }, addDay: { width: 112, height: 78, borderRadius: 22, alignItems: "center", justifyContent: "center", gap: 3, backgroundColor: "#F7FAFF", borderWidth: 1, borderColor: "#DDE9F6" }, addDayText: { color: "#6688BB", fontSize: 9, fontWeight: "900" },
  timelineCard: { padding: 22, borderRadius: 29, backgroundColor: "#FFF", borderWidth: 1, borderColor: "#ECEEF4", boxShadow: "0 14px 34px rgba(67,71,104,.08)" }, timelineHead: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }, dayTitle: { color: PX.ink, fontSize: 19, fontWeight: "900" }, daySubtitle: { marginTop: 3, color: PX.muted, fontSize: 10, fontWeight: "600" }, weather: { paddingHorizontal: 11, paddingVertical: 7, borderRadius: 16, flexDirection: "row", alignItems: "center", gap: 5, backgroundColor: "#F4F8FD" }, weatherText: { color: "#607694", fontSize: 10, fontWeight: "800" },

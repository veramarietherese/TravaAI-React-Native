...
export const grad = ["#8EC7FF", "#B7B5FF", "#FFAFD0"] as const;
export const gradStrong = ["#78B5F7", "#9FA7F7", "#F193BE"] as const;
  ...
  return <View style={styles.headerWrap}>
    ...
    <View style={styles.headerTop}>
      ...
      <View style={styles.headerActions}>
        <CircleButton label="Travel group" onPress={() => router.push(`/trip/${tripId}/members` as Href)}><Ionicons name="people-outline" size={20} color={PX.ink}/></CircleButton>
...
export function DetailBottomNav() {
  ...
  return <View style={[styles.bottomOuter, { bottom: Math.max(10, insets.bottom + 4), pointerEvents: "box-none" }]}>
    <Glass style={styles.bottomBar}>
      <BottomItem label="Home" icon="home-outline" onPress={() => router.replace("/(traveler)/(tabs)/home" as Href)} />
      <BottomItem label="Trips" icon="airplane-outline" active onPress={() => router.replace("/(traveler)/(tabs)/trips" as Href)} />
      <Pressable accessibilityLabel="AI assistant" onPress={() => router.push("/(traveler)/(tabs)/ai" as Href)} style={({ pressed }) => [styles.aiPress, pressed && styles.pressed]}><LinearGradient colors={grad} style={styles.ai}><Ionicons name="sparkles" size={24} color="#FFFFFF"/><Text style={styles.aiLabel}>AI</Text></LinearGradient></Pressable>
      <BottomItem label="Discover" icon="compass-outline" onPress={() => router.push("/(traveler)/(tabs)/explore" as Href)} />
      <BottomItem label="Profile" icon="person-outline" onPress={() => router.push("/(traveler)/(tabs)/profile" as Href)} />
    </Glass>
  </View>;
}
function BottomItem({ label, icon, active, onPress }: { label: string; icon: TravaIconName; active?: boolean; onPress?(): void }) {
  return <Pressable accessibilityRole="button" accessibilityLabel={label} onPress={onPress} style={({ pressed }) => [styles.bottomItem, pressed && styles.bottomPressed]}><Ionicons name={icon} size={22} color={active ? "#6F8FEF" : "#30436B"}/><Text style={[styles.bottomLabel, active && styles.bottomActive]}>{label}</Text></Pressable>;
}
...
export function Soft3DIcon({ colors, glyph, icon, size = 62 }: { colors: readonly [string, string, ...string[]]; glyph?: string; icon?: TravaIconName; size?: number }) {
  return <View style={[styles.iconShadow, { width: size, height: size, borderRadius: size * .28 }]}>
    <LinearGradient colors={colors} start={{ x: .15, y: .05 }} end={{ x: .85, y: .95 }} style={[styles.iconTile, { width: size, height: size, borderRadius: size * .28 }]}>
      <View style={styles.iconHighlight}/>{icon ? <Ionicons name={icon} size={Math.round(size * .42)} color="#FFFFFF"/> : <Text style={[styles.iconGlyph, { fontSize: size * .38 }]}>{glyph}</Text>}
    </LinearGradient>
  </View>;
}
...
const styles = StyleSheet.create({
  ...
  iconShadow: { alignItems: "center", justifyContent: "center", boxShadow: "0 12px 24px rgba(96,110,150,.14)" }, iconTile: { overflow: "hidden", alignItems: "center", justifyContent: "center", borderWidth: 1, borderColor: "rgba(255,255,255,.9)" }, iconHighlight: { position: "absolute", left: 9, right: 9, top: 8, height: 13, borderRadius: 9, backgroundColor: "rgba(255,255,255,.28)" }, iconGlyph: { color: "#FFF", fontWeight: "900" },

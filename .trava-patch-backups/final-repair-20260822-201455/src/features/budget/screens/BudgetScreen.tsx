...
export function BudgetScreen() {
  ...
  return <SafeAreaView style={s.safe} edges={["top"]}><StatusBar style="dark"/><ScreenShell tripId={tripId} title={trip.name || "Japan"}>
    ...
    <ExpenseModal key={editor === "new" ? "new" : editor?.id ?? "closed"} value={editor} onClose={() => setEditor(null)} onSave={(v) => { if (editor && editor !== "new") updateExpense(editor.id, v); else addExpense({ ...v, shared: false, paid: true }); setEditor(null); }}/>
